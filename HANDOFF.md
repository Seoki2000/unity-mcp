...
dev-0.0.2 절이 뒤에 붙어 있다. 결론 한 줄은 그대로다: 이 작업은 토큰을 줄인 것이 아니라
...
| 2c | (2026-08-24) | YAML 값 파서 + `unity_get_asset_components`, 맨 GUID 참조, 캐시/목록 수정 |

### 인덱스 성능 (MainProject, Unity 6000.3.16f1) — 2026-08-24 재측정
```
레이어 A    3,000 ms   수집 0.5s / .meta 3142 0.8s / YAML 1144파일 181MB 1.8s / edges 6043
레이어 B      550 ms   DLL → 사용자 어셈블리 14 / 타입 1169 / 소스 560
호출 그래프    70 ms   메서드 7170 디코딩, 실패 0, 엣지 8673
────────────────────
전체        3,900 ms   콜드
캐시 로드     690 ms   대부분이 지문 재계산(파일 4,286개 stat)
...
→ `tools/list` 총 82개 (Unity 73 + 로컬 9), 39,589 B
...
- [ ] **확장자 허용목록의 구조적 한계** — `YAML_EXT` 를 24종으로 늘렸지만 서드파티 자산
      ...
      **참고**: 2026-08-24 에 *형태* 화이트리스트는 같은 논리로 버렸다(§4-10). 확장자 쪽도
      같은 방향이 맞다 — 비용만 재면 된다.
...
### 회귀 기준 (2026-08-24 재측정)
- Missing Script: `coverage=full`, missing **14**, 영향 에셋 **134**, refs **164**, 상위 3건 합계 **132**
  (부분 커버리지에서는 missing 0 / `exists: null` 4,667 이 정상이다 — §4-12)
- 호출 그래프: `BaseAttack::TryResolveHit` → 호출자 **8개**
- IL 디코딩 실패 **0**, 엣지 **8,673**
- 참조 엣지 **6,043** (그중 맨 GUID 로만 얻은 것 **22**)
- 값 파서 전수: 문서 **69,891** 개, 못 읽은 줄 **0**, 정규식 대조군 누락 **0**
- 전수 스윕(전체 커버리지): 스크립트 컴포넌트 5,418 / 타입 해석 **751** / 패키지라 미해석 **4,397** /
  없는 스크립트 **270** / `m_EditorClassIdentifier` 불일치 **11**(전부 클래스 이름 변경, §4-10 참조)
- `tools/list` **82개 / 39,589 B**
- `Tools/prototype/` 의 프로토타입과 수치가 일치해야 한다

재현 스크립트는 커밋에 넣지 않았다. 위 수치는 `Bridge/index/*.js` 를 직접 require 해서
프로젝트 전체를 도는 짧은 노드 스크립트로 냈다(`getAssetComponents` 를 파일마다 1회 호출).
