...
최종 갱신: 2026-08-27 (P3-a·SequencePoints·search folder·중첩 타입·P3-b·**필드 타입 서명**)
...
| 프로브 | `probe-field-types` **8/8** · `probe-error-impact` **11/11** · `probe-nested-types` **8/8** · `probe-method-lines` **7/7** · `probe-verify-loop` **7/7** · `probe-impact-analysis` **10/10** · `probe-project-map` 통과 |
...
- `Tools/probe-field-types.js` — 필드 타입 서명 프로브 8개 (신규)
...
- [ ] **오버로드 구분** — 호출 그래프 키가 `Type::Method` 라 오버로드가 여전히 **합쳐진다.**
      진짜 분리는 메서드 시그니처 디코딩이 필요하다(ECMA-335 II.23.2.1).
      2026-08-27 에 **모호성을 드러내는 것까지는** 했다(`ca434fa`) — `find_callers` 가
      `declarations` 로 선언 수와 위치를 싣고, 선언이 하나뿐이면 경고가 아예 안 붙는다.
      실측으로 드러난 것: `BaseAttack::TryResolveHit` 의 호출자 8개는 **오버로드 4개에 걸친
      합계**였고(선언 130/153/165/176행), `Unit::TakeDamage` 는 선언 2개(62, 147)가 9개로 합쳐진다.
      "이름 바꿀 수 있나" 에는 현재로도 충분하다 — 어느 오버로드인지까지 알아야 할 때만 필요하다.
