...
### 🔌 넘겨받는 시점의 상태 (2026-08-26 02:1x 확인)
...
| 오프라인 동작 | 방금 확인: 캐시에서 인덱스 로드 → 영향 분석 응답 정상, 프로브 **10/10** 과 **6/7** 그대로 |

즉 **P3 가 아닌 작업은 Unity 를 안 켜도 바로 시작할 수 있다.** 켜야 하는 경우는
(a) P3 검증 루프, (b) C# 을 고쳤을 때, (c) 새 파일의 `.meta` 가 필요할 때, (d) 대조군.

```bash
"C:\Program Files\Unity\Hub\Editor\6000.3.16f1\Editor\Unity.exe" -projectPath C:\Unity\MainProject
```
