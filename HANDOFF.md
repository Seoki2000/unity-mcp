...
최종 갱신: 2026-08-27 (P3-a 진단 정직성·인덱스 신선도 + PDB SequencePoints, **새 도구 0개**)
...
## 0.5 다음 세션은 여기서 시작한다 — 결정 대기 (P3-b / 부채 / 종료)
...
| `tools/list` | 84개 / **42,142 B — P3-a 로 안 늘었다** |
| 프로브 | `probe-method-lines` **7/7** · `probe-verify-loop` **7/7** · `probe-impact-analysis` **10/10** · `probe-project-map` 통과 |
...
- `Tools/probe-method-lines.js` — 메서드 위치·오버로드 가시성 프로브 7개 (신규)
...
### 다음 작업 — 지정된 것이 없다. 사용자가 고른다

로드맵(Phase 3 P0~P3)은 **끝났다.** P3-b 를 포함해 남은 것은 전부 선택 항목이다.

1. **여기서 닫는다.** 지금 도구들은 모르는 것을 "모른다" 고 답한다. 아무것도 안 해도 된다
2. **P3-b — 복합 조인 도구.** 만들기 전에 아래 조건을 먼저 갖출 것:
   - ~~PDB SequencePoints 디코딩~~ **완료**(`d0714ad`). 다만 `line` 은 본문 시작이라
     **시그니처 줄의 오류는 어느 메서드 범위에도 안 들어간다.** 선언 줄까지 위로 넓히거나
     "다음 메서드에 귀속" 규칙을 먼저 정할 것. 정하기 전에는 `method: unknown` 을 답한다
   - 컴파일 세대와 인덱스 세대를 같이 실어 `freshness: current | last-good | stale | unknown`
     을 명시. 모르면 조인하지 않는다
   - **A(기존 도구 응답 가공)가 아니라 B(별도 브릿지 도구)로 격리한다.** 실측 후보 정의
     559 B (약 151 ±45 토큰/세션, 재구성치). 근거는 §4-(25)
   - 착수 전에 클라이언트 팬아웃이 실제로 아픈지 관측할 것. 아프지 않으면 만들지 않는다
3. **정확도 부채** — 중첩 타입 이름 충돌 123개 / 오버로드 구분 / 필드 서명 디코딩.
   각 1세션, **조인 전체를 흔든다**
4. **중복 제거** — `unity_search_project` 를 인덱스 기반으로 + `folder` 파라미터 버그. 반 세션
5. **`effectByOperation` 실검증** — §3. P3-b 를 하면 하네스가 겹치므로 같이 하면 싸다
...
- [ ] **중첩 타입 이름 충돌** (2026-08-26 발견) — 전체 이름이 `namespace.name` 이라 선언 타입이
      빠지고, 그래서 서로 다른 클래스 안의 같은 이름이 겹친다. 실측: 사용자 어셈블리 **123개**,
      **전부 중첩 타입**(Assembly-CSharp 안에서는 `<>c` 11 / `PassData` 3 / `Segment` 2).
      `Outer/Inner` 로 만들면 구분되지만 **호출 그래프 키까지 바뀐다** — 조인 전체를 흔드는
      변경이므로 별건. 지금은 모호성만 드러낸다(`unity_impact_analysis` 거절 /
      `unity_get_type_symbols` 의 `ambiguousFullName`). 고칠 때 같이 볼 것:
      `callgraph.js` 의 타입 이름 생성과 `typeNameRefs` 의 매칭이 같은 규칙을 써야 한다.
