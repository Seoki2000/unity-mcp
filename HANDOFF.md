...
그래서 P3 를 둘로 쪼개 **P3-a 만 했다 — 새 도구 0개, `tools/list` 42,142 B 불변.**
① 인덱스 신선도 재검증(아래), ② 컴파일 진단 완결성, ③ 콘솔 절단 회계, ④ 오류 출처 분류.
복합 도구(P3-b)는 보류했다: 만들더라도 A 가 아니라 **별도 브릿지 도구(B)** 로 격리한다.
...
## 0.5 다음 세션은 여기서 시작한다 — 백로그 소진, 새 판단 대기

2026-08-27 세션이 **P3-a** 를 끝냈다. 밀린 검증은 없다.
`tools/list` 는 84개 / 42,142 B **그대로다** — P3-a 는 도구를 하나도 늘리지 않았다.

### 넘겨받는 시점의 상태 (2026-08-27 확인)
...
| 항목 | 결과 |
|---|---|
| 전체 에셋 지문 비용 | **678~702 ms** (5,825 파일) — 매 호출에 못 붙인다 |
| 어셈블리 서명 비용 | **11~18 ms** (378 파일) — 붙일 수 있다. 그래서 2단 검사다 |
| 서명이 재컴파일에 반응 | 실측 hash `-960737942` -> `346096321`, bytes `50520724` -> `50526952` |
| 무변경 재컴파일 | Editor.log `2182 evaluated, **1 updated**` — 사용자 어셈블리 0개 컴파일 |
...
| 이 프로젝트의 CS 경고 | Editor.log 전체에 `warning CS` **0건** — 진짜로 경고 없이 컴파일된다 |
...
| `tools/list` **실측** | **85개 / 43,368 B / 도구당 510.2** — 브릿지를 띄워 클라이언트가 보는 값을 쟀다. 정의 원본을 더한 43,392 는 추정치였다(정규화 때문에 다르다) |
| 두 참조 검색 경로 | 같은 대상 `AudioManager.cs`: `search_project(reference)` 3,143 스캔 **1,344 ms** vs `find_references` **14 ms** (96배). 답은 둘 다 2건 |
...
| 캐시 버전 | **12** (시그니처 그래프). 캐시 4,464,535 B |
| 필드 서명 디코딩 | **전** 어셈블리(189개) 필드 **105,972 실패 0건**, 순수 디코딩 68 ms. **사용자** 어셈블리는 5,920개(중복타입 포함) / 5,554개(중복제거) — 앞 수치와 모집단이 다르다. 디스크 대조는 표본 150/150 이고, **소스 파일이 매핑되지 않은 타입의 필드 1,052개는 대조 자체가 불가능**하다 |
| 콜드 빌드 기준선 | **4.77~4.92 s** (Unity 유휴, 3회). 이전 기록 7.15~7.45 s 는 Unity 재컴파일 중에 잰 값이라 무효 |
| 캐시 크기 | **3,695,503 B** (필드 타입으로 +247,009, +7.2%) |
| 컴파일 실패 시 어셈블리 | **갱신되지 않는다.** 실제 실패를 만들어 확인 — 서명이 바이트 단위로 동일(hash 2042387871) |
| 컴파일러가 내는 경로 | **역슬래시 + 프로젝트 상대**(`Assets\_McpProbeTemp\X.cs`). 절대 경로도 올 수 있다 |
...
- `Tools/sweep-field-checks.js` — §6 전수 집계 재측정(측정 시임 `queries._checkFields` 사용) (신규)
...
- `Tools/probe-project-map.js` — 지도 프로브 7개
...
5. ~~`unity_search_project` 를 인덱스 기반으로 교체~~ — **닫았다**(2026-08-27).
   folder 버그를 고치니 타임아웃이 사라지고, 실측하니 두 경로가 **경계에서 갈린다**
   (`GetDependencies` 는 VFX Graph 내부 참조를 놓치고 역 GUID 인덱스는 런타임 경로 조회를
   못 본다). 하나를 지우면 교차검증의 짝이 없어진다(§4-(31)). 대신 응답이 **왜 둘이
   있는지와 어느 쪽이 빠른지**를 말하게 했다(`c1080ba`). 되돌리려면 96배 수치가 근거다
...
- [ ] **Phase 3 P3-b — 조인 복합 도구** (보류, 2026-08-27). 원안의 "오류 줄 -> 메서드 -> 호출자"
      의 **절반이 채워졌다** — 줄번호->메서드는 `d0714ad` 로 생겼다(단 본문 범위, 시그니처 아님).
      남은 장벽은 **컴파일 실패 시 어셈블리가 안 갱신돼 인덱스가 낡는 것**이다. 착수 조건은 §0.5 에 적어뒀다.
...
- [x] ~~**PDB SequencePoints 디코딩**~~ — 완료(2026-08-27, `d0714ad`). `MethodDebugInformation`
      ...
      716개 없음 / 오류 0, 디스크 대조 표본 40/40. 비용 콜드 빌드 5.66 -> 7.15~7.45 s,
...
- [ ] **`effectByOperation` 실검증** (2026-08-26 추가) — 지금 값은 직렬화 형태에서 **도출한 것**이고
      Unity 동작을 실험한 결과가 아니다. 진짜 검증은 워크트리 복사본에 변형을 하나씩 적용하고
      (이름변경 / 이동 / 삭제 / 서명 변경 / 본문 변경) 재컴파일해 진단·깨진 참조·`<Missing>`
      배선·Behavior 그래프 오류·테스트 결과를 관측하는 것이다. 독립 감사가 제안한 방법이고
      동의했다. 별도 하네스가 필요하며 P3(검증 루프)와 재료가 겹친다 — 같이 하면 싸다.
