...
최종 갱신: 2026-08-27 (Phase 3 P3-a — 진단 정직성 + 인덱스 신선도, **새 도구 0개**)
...
| 항목 | 상태 |
|---|---|
| 리포 | 워킹트리 깨끗, `optimized` = `9a76e66`. P3-a 커밋 6개(`8f80fce` -> `9a76e66`) — 코드 4개 + 문서 2개. **원격 푸시 여부는 아래 참조** |
...
| 프로브 | `probe-verify-loop` **7/7** · `probe-impact-analysis` **10/10** · `probe-project-map` 통과 |
...
| 인덱스에 줄번호->메서드 | **없다.** `symbols.js` 가 PDB 의 Document 컬럼만 읽는다. SequencePoints 미디코딩 |
...
- `Tools/probe-verify-loop.js` — 신선도·절단 프로브 7개 (신규)
...
2. **P3-b — 복합 조인 도구.** 만들기 전에 아래 조건을 먼저 갖출 것:
   - PDB SequencePoints 디코딩(줄번호->메서드). **없으면 `method: unknown` 을 답해야 하고,
     옛 바이너리에서 추측하면 안 된다**
...
- [ ] **Phase 3 P3-b — 조인 복합 도구** (보류, 2026-08-27). 원안의 "오류 줄 -> 메서드 -> 호출자"
      는 **지금 데이터로 못 만든다** — 인덱스에 줄번호->메서드가 없고(PDB SequencePoints 미디코딩),
      컴파일 실패 시 어셈블리가 안 갱신돼 인덱스가 낡는다. 착수 조건은 §0.5 에 적어뒀다.
      만들더라도 기존 도구 응답 가공(A)이 아니라 **별도 브릿지 도구(B)** 로 격리한다 — §4-(25).
- [ ] **PDB SequencePoints 디코딩** (2026-08-27 추가) — `symbols.js` 가 `MethodDebugInformation`
      의 Document 컬럼만 읽는다. SequencePoints 를 디코딩하면 **줄번호 -> 메서드**가 생기고
      P3-b 의 전제가 채워진다. 오버로드 구분과도 재료가 겹친다(둘 다 메서드 신원 문제).
...
- [ ] **오버로드 구분** — 호출 그래프 키가 `Type::Method` 라 오버로드가 합쳐진다.
      ...
      실측 예(2026-08-26): `Unit::TakeDamage` 는 선언 2개가 한 키에 호출자 9개로 합쳐진다.
