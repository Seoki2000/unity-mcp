...
최종 갱신: 2026-08-25 (라이브 교차검증 + 타입이름 축 오답 수정, `1102330`, `2.3.0-dev.0.0.3`)
...
## 0.5 ⏭️ 다음 세션은 여기서 시작한다 — Phase 3 P1 (프로젝트 맵)

2026-08-25 세션이 §0.5 의 교차검증을 끝냈다. **밀린 검증은 없다.** 다음은 기능이다.

### 지난 세션에 확인된 것 (다시 재현하지 말 것)

| 항목 | 결과 |
|---|---|
| 라이브 `tools/list` (Unity 병합 경로) | **82개 / 39,669 B** — 캐시 경로와 바이트 동일 |
| 새 엣지 5종 vs `GetDependencies` | **예상 5개 전부 적중** (보고서 §11.2) |
| 역방향 전수 대조 4건 | `SkillRange.png` `Boss_23_base.mat` 는 Unity 와 **동일 집합**, `BombAction.cs`(타입이름) `MainCamera.prefab`(경로상수)은 우리만 본다 |
| `search_project(type=reference)` 실측 | 에셋 **24,233 경로**, 콜드 **41초** → **에디터 큐 30초** 타임아웃에 걸린다(브릿지 45초가 아니다) |

대조군 계측기는 남겨뒀다 — `Editor/Debug/DependencyCrossCheck.cs`,
메뉴 `Window/MCP Dependency Cross-Check`, 입출력은 `~/.unity-mcp/depcheck-{in,out}.json`.
MCP 도구가 아니므로 `tools/list` 는 그대로다. 앞으로 새 엣지를 주장하면 여기에 쌍을 넣어 대조한다.

### 다음 작업

**Phase 3 P1 — `unity_project_map`.** 제안서 `C:/dev/unity-mcp-phase3-plan.md` §4, 그리고 §3 목록.
호출 그래프 8,673 엣지 + 참조 그래프 6,267 엣지에 중심성 랭킹과 토큰 예산을 얹는다.

시작하기 전에 §4-(21) 을 읽을 것. 이번 세션의 교훈은 **"새 축을 인덱스에 넣을 때 기존 도구의
0 을 같이 고치지 않으면 데이터는 있는데 답은 여전히 0"** 이다. P1 도 같은 함정이 있다 —
맵에 새 랭킹을 넣으면 기존 도구들이 그 순위를 모른 채 답한다.
...
- 브릿지 호출 하네스가 없으면 stdio JSON-RPC 로 직접 물으면 된다. 지난 세션에 쓴 스크립트:
...
- [ ] **Phase 3 P1 — 프로젝트 맵(`unity_project_map`)** — 제안서 §4.
      호출 그래프 8,673 엣지 + 참조 그래프 6,267 엣지에 랭킹(중심성)과 토큰 예산을 얹어
      "이 프로젝트에서 중요한 것부터 N 토큰만큼" 을 낸다. aider repo map 의 설계를 가져오되
      tree-sitter 는 쓰지 않는다(우리는 컴파일된 어셈블리를 본다).
- [ ] **Phase 3 P2 — 영향 분석(`unity_impact_analysis`)** — P0 가 끝나 이제 정확해질 수 있다.
      호출자 + 인스펙터 배선 + 타입 이름 참조 + 에셋 참조 + 씬 배치를 합쳐 영향 트리로.
