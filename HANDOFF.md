...
최종 갱신: 2026-08-26 (Phase 3 P1 — `unity_project_map`, `2.3.0-dev.0.0.4`)
...
## 0.5 ⏭️ 다음 세션은 여기서 시작한다 — Phase 3 P2 (영향 분석)

2026-08-26 세션이 P1(`unity_project_map`)을 끝냈다. 밀린 검증은 없다.

### 지난 두 세션에 확인된 것 (다시 재현하지 말 것)

| 항목 | 결과 |
|---|---|
| 라이브 `tools/list` | 83개 / **40,905 B** (P1 이전 82개 / 39,669 B) |
| 새 엣지 5종 vs `GetDependencies` | 예상 5개 전부 적중 (보고서 §11.2) |
| `search_project(type=reference)` | 에셋 **24,233 경로**, 콜드 **41초** → 에디터 큐 **30초** 타임아웃 |
| P1 랭킹 채점 (커밋 188건, 누수 차단) | 중심성 hit@50 **28.2%** < 부착 33.0% < churn 33.5% < attachThenChurn **35.1%**, 기준선 19.1% |
| P1 인수 프로브 | 4,000 → 5/7 · **6,000 → 6/7** · 8,000 → 7/7 (`node Tools/probe-project-map.js`) |
| 호출 엣지 분해 | 8,673 = 같은 타입 4,119 / **같은 파일 다른 타입 451** / 파일 간 3,834 / 미매핑 269 |
| 참조 엣지 해석 | 6,267 = **해석 4,421 / 인덱스 밖(패키지·빌트인) 1,846** |

계측기 둘을 남겨뒀다:
- `Editor/Debug/DependencyCrossCheck.cs` — 메뉴 `Window/MCP Dependency Cross-Check`,
  입출력 `~/.unity-mcp/depcheck-{in,out}.json`. 새 엣지를 주장하면 여기에 쌍을 넣어 Unity 와 대조한다.
- `Tools/probe-project-map.js` — 지도 인수 프로브 7개. 기본 예산에서 6 미만이면 종료코드 1.

### 다음 작업

**Phase 3 P2 — 영향 분석(`unity_impact_analysis`)** — §3 목록 참조.
"이 타입/메서드/에셋을 바꾸면 무엇이 깨지나" 를 호출자 + 인스펙터 배선 + 타입이름 참조 +
에셋 참조 + 씬 배치로 합쳐 낸다. P0/P1 이 끝나 재료는 다 있다.

시작하기 전에 **§4-(21) 과 (22) 를 같이 읽을 것.** 두 세션 연속으로 같은 실수를 했다 —
축을 인덱스에 넣고 그 축이 실리지 않는 응답을 남긴 것. P2 는 축 5개를 한 응답에 합치는
작업이므로 같은 실수를 할 자리가 다섯 개다. P1 에서 그걸 잡은 것은 리뷰가 아니라
**프로브(디스크에서 독립 확인 가능한 질문)** 였다. P2 도 프로브를 먼저 쓰고 시작할 것.

P2 에 쓸 수 있는 것 중 아직 안 쓴 재료: `weakRefs`(텍스트 일치 58건), 경로 로드 엣지 23,
`dynamicLoadSites` 44(정적으로 못 푼 것 — 영향 분석에서 "모른다" 로 정직하게 셀 자리).
...
- 인덱스 도구는 Unity 없이도 동작한다(`Tools/probe-project-map.js` 도 그렇다).
  켜는 이유는 **대조군** 때문이지 기능 때문이 아니다
...
- **JS 만 고쳤으면 Unity 를 안 켜도 된다.** 브릿지는 프로세스마다 새로 로드된다.
  C# 을 고쳤을 때만 Refresh + 컴파일 대기가 필요하다
...
### 로컬 인덱스 도구 10개 (브릿지가 Unity 를 거치지 않고 응답)
...
`unity_find_callers` / `unity_find_callees` / `unity_get_asset_components` / `unity_project_map`
→ `tools/list` 총 83개 (Unity 73 + 로컬 10), **40,905 B**
(레이어 D 시점 82개 / 39,669 B — P1 이 도구 1개를 늘려 +1,236 B ≈ +334 토큰/세션)
