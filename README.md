...
> Git URL 방식은 패키지가 `Library/PackageCache/com.community.unity-mcp@<해시>/`에 캐싱되며, 이 `<해시>`는 갱신 때마다 바뀝니다. MCP 연결에서 경로 문제가 생기면 아래 **방법 2** 또는 **"MCP 클라이언트 설정"**을 참고하세요.

### 방법 2 — 임베드 (적극 개발 / 안정 경로, 권장)

패키지를 직접 수정하거나 변하지 않는 경로가 필요하면 프로젝트의 `Packages/` 아래에 임베드하세요:
```bash
git clone https://github.com/Seoki2000/unity-mcp.git Packages/com.community.unity-mcp
```
임베드하면 코드 수정이 에디터에 바로 반영되고 경로에 `@해시`가 붙지 않습니다. 이 경우 `manifest.json`의 Git URL 항목은 제거하세요.

## 빠른 시작

1. Unity가 로드되면 서버가 **자동 시작**됩니다 (기본 포트 3000)
2. `Window > MCP Server`로 상태 확인
3. MCP 클라이언트를 아래 **"MCP 클라이언트 설정"** 대로 연결 (변하지 않는 경로 사용 권장)
...
> ⚠️ **흔한 함정 — `@해시` 경로**: Git URL로 설치하면 브릿지가 `Library/PackageCache/com.community.unity-mcp@<해시>/Bridge/mcp-bridge.js`에 위치하는데, `<해시>`는 패키지가 갱신될 때마다 바뀝니다. 이 경로를 설정에 그대로 박으면 **연결이 주기적으로 끊깁니다**("Copy Config to Clipboard"도 이 경로를 복사함). 아래처럼 **변하지 않는 경로**를 쓰세요.

브릿지(`Bridge/mcp-bridge.js`)는 **의존성 없는 단일 Node 파일**이고 `localhost:3000`(에디터 내 서버)으로 프록시만 합니다. 즉 어디에 두든, 어느 Unity 프로젝트가 열려 있든 동작합니다.

### 권장 A — 임베드 + 프로젝트 `.mcp.json` (단일 프로젝트)

패키지를 `Packages/com.community.unity-mcp/`에 임베드(설치 방법 2)한 뒤 프로젝트 루트에 `.mcp.json`:
```json
{
  "mcpServers": {
    "unity": {
...
```
상대경로라 `@해시`가 없고, 프로젝트와 함께 버전관리·이동됩니다.

### 권장 B — 공유 브릿지 + 전역 등록 (프로젝트 여러 개)

브릿지는 프로젝트와 무관하므로 한 곳에 복사해 한 번만 전역 등록하면 **모든 프로젝트**에서 쓸 수 있습니다(예: Claude Code):
```bash
# 1) 브릿지를 고정 위치에 복사 (한 번만). 예: ~/.unity-mcp/mcp-bridge.js
# 2) user 스코프로 전역 등록 — 모든 프로젝트에 적용
claude mcp add --scope user unity -- node "<고정경로>/mcp-bridge.js"
```
새 프로젝트를 만들어도 추가 설정 없이, Unity만 켜져 있으면 바로 연결됩니다.

> **포트 변경 시**: `Window > MCP Server`에서 포트를 바꿨다면 설정에 `"env": { "UNITY_MCP_PORT": "3000" }`를 추가하세요(기본값 3000).
> **연결 조건**: 도구가 실제 동작하려면 대상 Unity 에디터가 **열려 있어야** 합니다(브릿지는 프록시일 뿐). 서버는 로드 시 자동 시작되며 `Window > MCP Server`에서 상태를 확인합니다.
