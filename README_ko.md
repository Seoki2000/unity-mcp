...
패키지 버전: `2.3.0-dev.0.0.7` · 기준 커밋: `5331a34`

### dev-0.0.7 (2026-08-28) — 기능이 아니라 정확도

도구를 늘리지 않았다. `tools/list` 는 바이트 단위로 같다. 바뀐 것은 **답의 내용**이다.

- **메서드 줄 범위가 27.4% 틀려 있었다.** 음수 줄번호를 가진 메서드가 126개 있었다.
  Portable PDB 시퀀스 포인트 디코더가 상태 변수 하나로 두 가지를 표현해서, 숨은 포인트
  뒤의 **절대 줄**을 부호있는 델타로 읽었다. 이제 전수 대조가 프로브에 들어가 있다 —
  선언 대조 5,313/5,313, 겹치는 범위 0, 음수 0. 독립 .NET 참조 판독기와도 5,960개 전부 일치.
  `endLine` 은 마지막 포인트의 **끝 줄**로 고쳤고(전에는 시작 줄이라 메서드 133개가 최대
  22줄 짧았다), 부분 클래스의 메서드가 다른 파일 뷰로 새지 않는다.
- **`unity_explain_compile_errors` 가 그 줄이 무엇인지 말한다.** `lineKind` 로 파일 수준 /
  타입·멤버 속성 / 타입 선언 / 필드 선언 / 본문 / 모름을 가른다. 예전에 뭉뚱그렸던 `gap` 의
  86% 를 설명하고, 필드 선언이면 **실제 필드와 그 타입까지 조인**한다. `using` 한 줄이
  엉뚱한 메서드에 붙는 일은 이제 없다.
- **개수 필드 두 개를 세는 것에 맞게 바꿨다**: `totalReferenceCount` → `assetGuidPairCount`,
  행 단위 `referenceCount` → `referencingAssetCount`.
- **`danglingLoads`** — 경로가 풀렸는데 그 자리에 에셋이 없는 로드 호출. 컴파일러도 콘솔도
  침묵한다. 이 프로젝트에 실제로 2건 있다. 에셋을 옮기면 정확히 이 흔적만 남는다는 것을
  Unity 를 켜서 확인했다.
- **PDB 문서가 없는 타입**(필드만 있는 데이터 컴포넌트)에 `unity_impact_analysis` 가
  "붙은 것 없다" 고 답하던 것을 고쳤다 — 파일명 규칙으로 폴백하고 그 근거를 밝힌다.
- 인덱스 캐시 버전 12 → 16. 구 캐시는 폐기된다(안 올리면 고친 답이 캐시 세션에서 안 먹는다).
...
| **dev-0.0.7** | **43,368** | **85** | **510.2** (도구 정의 불변 — 응답 필드만 늘었다) |

이전 핀 대비 **+48.5%** — ⚠️ 이 값은 **dev-0.0.1 시점**이다(37,067 B 기준).
**현재(dev-0.0.7)는 +73.7%** 다(43,368 / 24,968). 3.5~4 B/토큰으로 환산하면 세션당
+3,000~3,500 토큰. 도구는 dev-0.0.1 까지 14개, **현재까지는 67 -> 85 로 18개** 늘었고,
annotations 를 새로 달았고, 공통 도구 4개의 설명이 +345자 늘어난 결과다.
...
| 실측 (MainProject) | 값 |
|---|---|
| 파서 커버리지 | 텍스트 에셋 1,144개(측정 시점 값 — 현재 1,171. 프로젝트가 자란다) / 181 MB / 문서 69,891개 → **못 읽은 줄 0** |
...
인덱스 밖으로 나가는 엣지 1,864 / 안 읽은 바이너리 681 / 오버로드 병합 / 중첩 타입 중복 레코드 123(고유 이름 29) /
...
- **인덱스 증분 갱신 미구현.** 변경 파일만 다시 읽지는 않는다 — 낡음을 감지하면 **전체를 다시 만든다**
  (콜드 **5.7~6.9 s** — Unity 꺼짐 기준. 에디터가 유휴면 4.7 s 다. **조건을 같이 적을 것**).
  다만 dev-0.0.6 부터는 `ensureIndex` 가 호출마다 신선도를 검증하므로
...
3. 아래의 저장소 URL을 입력합니다 (또는 로컬 클론 경로 사용):
   ...
   `optimized` 가 이 포크의 정본 브랜치입니다 — 무엇이 다른지는 위 [dev-0.0.7](#포크-개발-버전--dev-007) 참조.
